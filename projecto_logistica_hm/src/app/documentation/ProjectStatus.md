...
| Documentos adjuntos por solicitud (pestaña "Documentos" del `SolicitudDetalleModal`): subida en cualquier estado (rol autenticado), listado con tamaño/fecha/subidor, descarga con signed URL, eliminación (subidor + admin + logística); bucket privado `solicitud-documentos` + tabla `solicitud_documento` (RLS sin políticas, acceso service-role), acciones y servicio con auditoría `subir_documento`/`eliminar_documento` | Solicitudes | 2026-09-09 | `IMPLEMENTADO` | Sí — `tsc --noEmit` y ESLint sin errores; migración `20260909_solicitud_documentos.sql` pendiente de aplicar en Supabase y validación funcional pendiente |
